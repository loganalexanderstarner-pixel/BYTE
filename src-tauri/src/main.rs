                ...
                for line in text.lines() {
                    if let Ok(parsed) = serde_json::from_str::<OllamaChunk>(line) {
                        if let Some(resp) = parsed.response {
                            // Find longest common prefix between last and resp
                            let mut i = 0;
                            let min_len = last.len().min(resp.len());
                            while i < min_len && last.as_bytes()[i] == resp.as_bytes()[i] {
                                i += 1;
                            }
                            ...
                            if !new_part.is_empty() {
                                let _ = app.emit("stream", new_part.to_string());
                            }
